...
void usage()
{
    crgMsgPrint( dCrgMsgLevelNotice, "usage: crgMultiCp [options] <filename>\n" );
    crgMsgPrint( dCrgMsgLevelNotice, "       options: -h      show this info\n" );
    crgMsgPrint( dCrgMsgLevelNotice, "                -cp <n> number of contact points\n" );
...
#define FULL_TEST    
    ...
    /* --- check for total number of contact points --- */       
    if ( nCp <= 0 )
    {
        crgMsgPrint( dCrgMsgLevelFatal, "At least one contact point must be defined!\n" );
        usage();
    }
    ...
    cpIds = crgCalloc( nCp, sizeof( int ) );
    
    for ( i = 0; i < nCp; i++ )
    {
        cpIds[i] = crgContactPointCreate( dataSetId );
    
        if ( cpIds[i] < 0 )
        {
            crgMsgPrint( dCrgMsgLevelFatal, "main: could not create contact point no. %d.\n", i );
            return -1;
        }
    }
    
    crgMsgPrint( dCrgMsgLevelNotice, "main: successfully created %d contact points.\n", i );
    
    /* --- get extents of data set --- */
    crgDataSetGetURange( dataSetId, &uMin, &uMax );
    crgDataSetGetVRange( dataSetId, &vMin, &vMax );
    
    /* --- exceed standard u range by 20% in each direction --- */
    uRange = uMax - uMin;
    uMin  -= 0.2 * uRange;
    uMax  += 0.2 * uRange;
    
    /* --- compute the array of x/y contact points to be fed into the algorithms later on --- */
    noTestPts = ( int ) ( ( ( uMax - uMin ) * noPtsPatchWidth * noPtsPatchLength * noWheels / stepSizeU ) + 0.5 );
    ...
    for ( i = 0; i < nCp; i++ )
    {
        crgContactPointSetDefaultOptions( cpIds[i] );
        /* crgContactPointOptionsPrint( cpIds[i] ); */
    }
    
    crgMsgPrint( dCrgMsgLevelDebug, "main: generating %d test points\n", noTestPts );
    ...
    v         = 0.0;
    idxTestPt = 0;
    cpId      = cpIds[0];       /* use first contact point for preliminary calculation */
    ...
    while ( idxTestPt < noTestPts )
    {
        ...
        if ( !crgEvalxy2z( cpIds[cpIdx], testX[idxTestPt], testY[idxTestPt], &z ) ) {}
